...
class DynamicSectionGenerator:
    ...
    def generate_sections(
        ...
        sections = {
            ...
            "template_specific": self._generate_template_specific_section(
                template_type, market_snapshot, articles
            ),
        }
        ...
        if template_type == TemplateType.MARKET_CRASH:
            ...
            sections["policy_impact"] = self._generate_policy_impact_analysis(
                market_snapshot, articles
            )
    ...
    def _generate_market_overview(self, snapshot: MarketSnapshot) -> Dict[str, Any]:
        ...
        if snapshot.stock_indices:
            sorted_indices = sorted(
                snapshot.stock_indices, key=lambda x: x.change_percent, reverse=True
            )
        ...
        return {
            ...
            "total_instruments": len(
                snapshot.stock_indices + snapshot.currency_pairs + snapshot.commodities
            ),
    ...
    def _generate_article_insights(
        self, articles: List[Dict[str, Any]], template_type: TemplateType
    ) -> Dict[str, Any]:
        """記事洞察セクション生成"""
        if not articles:
        ...
        top_articles = sorted(
            articles,
            key=lambda x: (
                abs(x.get("sentiment_score", 0)) if x.get("sentiment_score") is not None else 0
            ),
            reverse=True,
        )[:5]
    ...
    def _generate_risk_assessment(
        self, snapshot: MarketSnapshot, articles: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """リスク評価セクション生成"""
        risk_level = "低"
        risk_factors = []
        ...
        # 大幅下落の確認
        major_declines = [idx for idx in snapshot.stock_indices if idx.change_percent <= -3.0]
        if major_declines:
    ...
    def _generate_template_specific_section(
        self, template_type: TemplateType, snapshot: MarketSnapshot, articles: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """テンプレート特有のセクション生成"""
        if template_type == TemplateType.MARKET_CRASH:
    ...
    def _generate_safe_haven_analysis(self, snapshot: MarketSnapshot) -> Dict[str, Any]:
        ...
        # 金の動向
        gold_data = next(
            (c for c in snapshot.commodities if "gold" in c.name.lower() or "金" in c.name), None
        )
        if gold_data:
            safe_havens.append(
                {
            ...
            safe_havens.append(
                {
                    ...
                    "status": (
                        "円高" if "JPY" in pair.symbol and pair.change_percent < 0 else "円安"
                    ),
                }
            )
    ...
    def _generate_policy_impact_analysis(
        self, snapshot: MarketSnapshot, articles: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """金融政策影響分析"""
        ...
        policy_articles = [
            ...
            for article in articles
            if any(
                keyword in (article.get("title", "") + " " + article.get("summary", "")).lower()
    ...
    def _generate_currency_outlook(self, snapshot: MarketSnapshot) -> Dict[str, Any]:
        ...
        for pair in snapshot.currency_pairs[:4]:
            trend = (
                "上昇"
                if pair.change_percent > 0.1
                else "下落" if pair.change_percent < -0.1 else "横ばい"
            )
    ...
    def _generate_risk_scenarios(
        ...
        geopolitical_articles = len(
            [
                ...
                for a in articles
                if any(
                    keyword in (a.get("title", "") + " " + a.get("summary", "")).lower()
