...
class TemplateSelector:
    ...
    def select_template(self, 
                       ...
                       datetime_context: Optional[datetime] = None) -> TemplateType:
        """
        ...
        self.logger.info(f"選択されたテンプレート: {selected_template.value} (スコア: {max_score:.2f})")
    ...
    def _calculate_template_score(self,
                                ...
                                market_snapshot: MarketSnapshot,
                                articles: List[Dict[str, Any]],
                                datetime_context: Optional[datetime]) -> float:
        """テンプレートスコアを計算"""
        ...
        if "negative_sentiment_threshold" in conditions:
            ...
            if market_snapshot.overall_sentiment == MarketTrend.BEARISH:
                ...
                if market_snapshot.stock_indices:
                    avg_change = sum(idx.change_percent for idx in market_snapshot.stock_indices) / len(market_snapshot.stock_indices)
        ...
        if "major_decline_threshold" in conditions:
            ...
            max_score += 0.3
            major_declines = [
                idx for idx in market_snapshot.stock_indices 
                if idx.change_percent <= threshold
            ]
        ...
        # 特定分野のキーワード
        for key in ["fed_keywords", "central_bank_keywords", "earnings_keywords", 
                   "geopolitical_keywords", "crypto_symbols"]:
        ...
        if "crypto_volatility_threshold" in conditions:
            max_score += 0.2
            crypto_data = [
                data for data in (market_snapshot.stock_indices + 
                                market_snapshot.currency_pairs + 
                                market_snapshot.commodities)
                if any(crypto_term in data.symbol.upper() 
                      for crypto_term in ["BTC", "ETH", "CRYPTO"])
            ]
            if crypto_data:
                avg_crypto_volatility = sum(abs(d.change_percent) for d in crypto_data) / len(crypto_data)
    ...
    def _calculate_keyword_score(self, articles: List[Dict[str, Any]], keywords: List[str]) -> float:
        """キーワードスコアを計算"""
    ...
    def _count_relevant_articles(self, articles: List[Dict[str, Any]], conditions: Dict[str, Any]) -> int:
        """関連記事数をカウント"""
        relevant_count = 0
        
        # 全てのキーワードを統合
        all_keywords = []
        for key in ["keywords", "fed_keywords", "central_bank_keywords", 
                   "earnings_keywords", "geopolitical_keywords", "crypto_symbols"]:
            if key in conditions:
