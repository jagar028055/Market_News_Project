...
class DatabaseManager:
    ...
    def get_recent_articles(
        self, hours: int = 24, include_analysis: bool = True, source: Optional[str] = None
    ) -> List[Article]:
        """
        最新記事取得
        ...
        with self.get_session() as session:
            ...
            query = session.query(Article).filter(Article.published_at >= cutoff_time)
    ...
    def detect_duplicate_articles(self) -> List[Tuple[Article, Article]]:
        ...
        with self.get_session() as session:
            ...
            duplicates = (
                session.query(Article)
                .filter(Article.content_hash.in_(subquery))
                .order_by(Article.content_hash, Article.scraped_at)
                .all()
            )
    ...
    def update_scraping_session(self, session_id: int, **kwargs) -> Optional[ScrapingSession]:
        """スクレイピングセッション更新"""
        with self.get_session() as session:
    ...
    def complete_scraping_session(
        self, session_id: int, status: str = "completed", error_details: Optional[str] = None
        ...
        with self.get_session() as session:
            ...
            if scraping_session.started_at:
                total_duration = (
                    scraping_session.completed_at - scraping_session.started_at
                ).total_seconds() * 1000
    ...
    def get_statistics(self, days: int = 7) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            ...
            # 基本統計
            total_articles = (
                session.query(Article).filter(Article.scraped_at >= cutoff_date).count()
            )
            ...
            recent_sessions = (
                ...
                .filter(ScrapingSession.started_at >= cutoff_date)
