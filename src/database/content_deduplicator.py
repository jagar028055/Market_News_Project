...
class ContentDeduplicator:
    ...
    def _normalize_content(self, content: str) -> str:
        ...
        filtered_words = [
            word for word in words if word.lower() not in self.noise_words and len(word) > 1
        ]
    ...
    def remove_duplicates(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for article in articles:
            ...
            # 詳細重複チェック
            is_duplicate = any(
                self.is_duplicate_article(article, existing) for existing in unique_articles
            )
            ...
            if not is_duplicate:
                ...
                self.logger.debug(
                    f"Duplicate detected by similarity: {article.get('title', '')[:50]}"
                )
        ...
        self.logger.info(
            f"重複除去完了: 元記事数={original_count}, ユニーク記事数={unique_count}, 重複記事数={duplicate_count}"
        )
