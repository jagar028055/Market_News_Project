...
try:
    # 実行環境に sentence-transformers が無い場合でも動作を継続
        ...
        if self._model is None:
            try:
                if SentenceTransformer is None:
                    self.logger.warning("sentence-transformers が見つからないため、モデルを初期化しません")
                    return None
                self.logger.info(f"埋め込みモデル読み込み中: {self.config.embedding_model}")
                self._model = SentenceTransformer(self.config.embedding_model)
                self.logger.info("埋め込みモデル読み込み成功")
    ...
    def validate_embedding(self, embedding: List[float]) -> bool:
        ...
        try:
            if np is not None:
                ...
                import math
                if any(not math.isfinite(float(val)) for val in embedding):
                    self.logger.warning("埋め込みに不正な値が含まれています")
                    return False
        except Exception:
            # 予期せぬエラー時は保守的にFalseにしない
            pass
