...
class EnhancedAISummarizer:
    ...
    def process_article_enhanced(
        self, text: str, title: Optional[str] = None, market_context: Optional[str] = None
    ) -> Optional[EnhancedSummaryResult]:
        """
        拡張記事処理
        ...
        if not text or len(text.strip()) < 50:
            self.logger.warning(
                f"記事本文が短すぎます (長さ: {len(text.strip()) if text else 0}文字)"
            )
            return None
    ...
    def _build_enhanced_prompt(
        self, text: str, title: Optional[str], market_context: Optional[str]
    ) -> str:
        """拡張プロンプト構築"""
...
"""
    ...
    def _parse_enhanced_response(
        self, response_text: str, has_market_context: bool
        ...
        try:
            ...
            result["quality_level"] = (
                quality_text if quality_text in ["high", "medium", "low"] else "medium"
            )
