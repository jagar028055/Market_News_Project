...
from .custom_exceptions import NewsAggregatorError, RetryableError, NonRetryableError, ErrorContext
...
class ErrorHandler:
    ...
    def handle_error(
        self, error: Exception, context: Optional[ErrorContext] = None, reraise: bool = True
    ) -> None:
        """エラーの処理とログ記録"""
    ...
    def create_context(
        self, operation: str, component: str, details: Optional[Dict[str, Any]] = None
    ) -> ErrorContext:
        """エラーコンテキストの作成"""
        return ErrorContext(
            operation=operation,
