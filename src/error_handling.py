...
def handle_requests_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            logger.error(f"Request timeout in {func.__name__}", extra={
                'extra_data': {'error_type': 'timeout', 'error': str(e)}
            })
            raise ScrapingError(f"Request timeout: {e}")
            
        except requests.exceptions.ConnectionError as e:
            logger.error(f"Connection error in {func.__name__}", extra={
                'extra_data': {'error_type': 'connection', 'error': str(e)}
            })
            ...
            if status_code and 400 <= status_code < 500:
                # 4xxエラーはリトライしない
                logger.error(f"Client error in {func.__name__}", extra={
                    'extra_data': {
                ...
                # 5xxエラーはリトライ可能
                logger.error(f"Server error in {func.__name__}", extra={
                    'extra_data': {
            ...
            logger.error(f"Request error in {func.__name__}", extra={
                'extra_data': {'error_type': 'request', 'error': str(e)}
            })
...
def handle_selenium_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            logger.error(f"Selenium timeout in {func.__name__}", extra={
                'extra_data': {'error_type': 'selenium_timeout', 'error': str(e)}
            })
            raise ScrapingError(f"Selenium timeout: {e}")
            
        except selenium.common.exceptions.NoSuchElementException as e:
            logger.warning(f"Element not found in {func.__name__}", extra={
                'extra_data': {'error_type': 'element_not_found', 'error': str(e)}
            })
            raise ScrapingError(f"Element not found: {e}")
            
        except selenium.common.exceptions.WebDriverException as e:
            logger.error(f"WebDriver error in {func.__name__}", extra={
                'extra_data': {'error_type': 'webdriver', 'error': str(e)}
            })
...
def handle_google_api_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            if status_code == 403:
                logger.error(f"Google API permission denied in {func.__name__}", extra={
                    'extra_data': {
                ...
                logger.error(f"Google API resource not found in {func.__name__}", extra={
                    'extra_data': {
                ...
                logger.error(f"Google API client error in {func.__name__}", extra={
                    'extra_data': {
                        'error_type': 'client_error',
                        'status_code': status_code,
                        'error': str(e)
                ...
                # 5xxエラーはリトライ可能
                logger.error(f"Google API server error in {func.__name__}", extra={
                    'extra_data': {
                        'error_type': 'server_error',
                        'status_code': status_code,
                        'error': str(e)
            ...
            logger.error(f"Unexpected Google API error in {func.__name__}", extra={
                'extra_data': {'error_type': 'unexpected', 'error': str(e)}
            })
            raise
...
def safe_execute(
    ...
    **kwargs
    ...
    try:
        ...
        if log_errors:
            logger.error(f"Error in safe_execute for {func.__name__}", extra={
                'extra_data': {
                    'function': func.__name__,
                    'error': str(e),
                    'error_type': type(e).__name__
