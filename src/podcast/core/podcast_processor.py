...
class PodcastProcessor:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.audio_processor = AudioProcessor(str(Path(config.base_dir) / "src" / "podcast" / "assets"))
    ...
    async def generate_daily_podcast(self) -> Dict:
        ...
        try:
            ...
            # Step 6: エピソード情報作成
            episode_info = await self._create_episode_info(processing_id, news_data, script, final_audio_path)
    ...
    async def _check_cost_limits(self) -> None:
        ...
        if current_costs.get('total', 0) + estimated_cost > self.podcast_config.monthly_cost_limit:
            raise PodcastProcessingError(
                f"予想コストが制限を超過します: ${estimated_cost:.2f}"
            )
            
        logger.info(f"Cost check passed. Current: ${current_costs.get('total', 0):.2f}, Estimated: ${estimated_cost:.2f}")
    ...
    async def _collect_and_summarize_news(self) -> Dict:
        ...
        try:
            ...
            summarized_articles = await asyncio.to_thread(
                self.news_processor.process_articles_with_ai, 
                collected_articles
            )
            ...
            podcast_news_data = {
                ...
                'sources': list(set(article.get('source', 'unknown') for article in summarized_articles))
            }
    ...
    async def _synthesize_audio(self, script: str) -> bytes:
        ...
        try:
            ...
            audio_data = await asyncio.to_thread(
                self.tts_engine.synthesize_dialogue,
                script
            )
    ...
    async def _create_episode_info(self, episode_id: str, news_data: Dict, 
                                 script: str, audio_path: str) -> Dict:
        """エピソード情報作成"""
    ...
    async def _publish_episode(self, episode_info: Dict, audio_path: str) -> Dict:
        ...
        try:
            ...
            publish_results = await asyncio.to_thread(
                self.publisher.publish_episode,
                episode_info,
                audio_path
            )
    ...
    def _create_processing_result(self, processing_id: str, episode_info: Optional[Dict],
                                publish_results: Optional[Dict], success: bool,
    ...
    def _create_episode_description(self, news_data: Dict, script: str) -> str:
        ...
        description += f"収集時刻: {news_data['collection_time'].strftime('%Y年%m月%d日 %H:%M')}\n\n"
    ...
    async def _extend_script(self, script: str, articles: List[Dict], target_length: int) -> str:
        ...
        if remaining_articles:
            additional_script = await asyncio.to_thread(
                self.script_generator.generate_script,
                remaining_articles[:3]  # 追加で3記事を使用
            )
    ...
    async def _compress_audio(self, audio_path: str) -> str:
        ...
        compressed_path = await asyncio.to_thread(
            self.audio_processor.compress_audio,
            audio_path,
            self.podcast_config.max_file_size_mb
        )
