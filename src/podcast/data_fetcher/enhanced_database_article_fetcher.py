...
class EnhancedDatabaseArticleFetcher:
    ...
    def get_articles_for_podcast(
        ...
        with self.db_manager.get_session() as session:
            ...
            if min_sentiment_score is not None:
                query = query.filter(func.abs(AIAnalysis.sentiment_score) >= min_sentiment_score)
            ...
            log_with_context(
                ...
                avg_importance_score=(
                    sum(a["importance_score"] for a in detailed_articles) / len(detailed_articles)
    ...
    def _calculate_importance_scores(self, articles: List[Article]) -> List[Tuple[Article, float]]:
        ...
        for article in articles:
            ...
            # 総合スコア
            total_score = (
                sentiment_weight + summary_weight + recency_weight + category_weight + region_weight
            )
    ...
    def _build_detailed_article_data(
        self, scored_articles: List[Tuple[Article, float]]
    ) -> List[Dict[str, Any]]:
        """
        詳細記事データ構築
        ...
        for article, importance_score in scored_articles:
            ...
            article_data = {
                ...
                "category_weight": self.CATEGORY_WEIGHTS.get(
                    analysis.category if analysis else "other", 1.0
                ),
                "region_weight": self.REGION_WEIGHTS.get(
                    analysis.region if analysis else "other", 1.0
                ),
                # 追加メタデータ
                "character_count": len(article.title or "")
                + len(analysis.summary if analysis else ""),
    ...
    def get_articles_by_category(
        ...
        return self.get_articles_for_podcast(
            hours=hours, max_articles=limit, category_filter=[category]
        )

    def get_articles_by_region(
        self, region: str, hours: int = 24, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        地域別記事取得
        ...
        """
        return self.get_articles_for_podcast(
            hours=hours, max_articles=limit, region_filter=[region]
        )

    def get_balanced_article_selection(
        self, hours: int = 24, max_articles: int = 6
    ) -> List[Dict[str, Any]]:
        """
        バランス取れた記事選択
        カテゴリ・地域の多様性を考慮した選択
        ...
        all_articles = self.get_articles_for_podcast(
            hours=hours, max_articles=max_articles * 3  # 候補を多めに取得
        )
    ...
    def get_database_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self.db_manager.get_session() as session:
            ...
            # 基本統計
            total_articles = (
                session.query(Article).filter(Article.published_at >= cutoff_time).count()
            )
            ...
            # センチメント統計
            sentiment_stats = (
                session.query(AIAnalysis.sentiment_label, func.count(AIAnalysis.id))
                .join(Article)
                .filter(Article.published_at >= cutoff_time)
            ...
            statistics = {
                ...
                "analysis_coverage_rate": (
                    analyzed_articles / total_articles if total_articles > 0 else 0
                ),
