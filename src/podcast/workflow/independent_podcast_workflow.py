...
class IndependentPodcastWorkflow:
    ...
    def tts_engine(self) -> GeminiTTSEngine:
        ...
        if self._tts_engine is None:
            self._tts_engine = GeminiTTSEngine(
                api_key=self.config.gemini_api_key
            )
    ...
    def audio_processor(self) -> AudioProcessor:
        ...
        if self._audio_processor is None:
            ...
            # ビットレート設定更新
            self._audio_processor.update_settings({
    ...
    def message_templates(self) -> MessageTemplates:
        ...
        if self._message_templates is None:
            self._message_templates = MessageTemplates(
                base_url=self.config.github_pages_base_url
            )
    ...
    async def execute_workflow(self) -> WorkflowResult:
        ...
        try:
            ...
            selected_articles = await self._execute_step(
                "記事データ解析",
                lambda: self._step_analyze_articles(articles_data),
                result
            )
            ...
            script = await self._execute_step(
                "台本生成",
                lambda: self._step_generate_script(selected_articles),
                result
            )
            ...
            audio_data = await self._execute_step(
                "音声合成",
                lambda: self._step_synthesize_audio(script, result.episode_id),
                result
            )
            ...
            audio_url = await self._execute_step(
                "GitHub Pages配信",
                lambda: self._step_publish_to_github(processed_audio_path, result.episode_id, selected_articles),
                result
            )
    ...
    async def _execute_step(self, step_name: str, step_func, result: WorkflowResult):
        ...
        self.logger.info(f"📍 ステップ開始: {step_name} ({self.progress.completed_steps + 1}/{self.progress.total_steps})")
    ...
    async def _step_initialization(self) -> None:
        ...
        try:
            ...
            is_accessible = self.gdrive_reader.validate_document_access(self.config.google_document_id)
    ...
    async def _step_process_audio(self, audio_data: bytes, episode_id: str, articles: List[Dict[str, Any]]) -> str:
        """ステップ6: 音声処理"""
        try:
            # メタデータ作成
            metadata = {
    ...
    async def _step_publish_to_github(self, audio_file_path: str, episode_id: str, articles: List[Dict[str, Any]]) -> Optional[str]:
        """ステップ7: GitHub Pages配信"""
        try:
            # エピソードメタデータ作成
            episode_metadata = {
    ...
    async def _step_send_notifications(self, result: WorkflowResult, articles: List[Dict[str, Any]]) -> None:
        """ステップ8: 通知送信"""
        try:
            # 通知メッセージ作成
            episode_data = {
    ...
    def get_component_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "audio_processor": {
                ...
                "info": self._audio_processor.get_processing_info() if self._audio_processor else None
            },
            "publisher": {
            ...
            "error_handler": {
                ...
                "stats": self._error_handler.get_error_statistics() if self._error_handler else None
            }
        }
