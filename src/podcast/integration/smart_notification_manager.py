...
class SmartNotificationManager:
    ...
    def send_podcast_notification(self, episode_info: Dict[str, Any], articles: List[Dict[str, Any]], 
        ...
        try:
            ...
            rss_url = send_options.get('rss_url', f"{self.config.podcast.rss_base_url}/podcast/feed.xml")
    ...
    def schedule_podcast_notification(self, episode_info: Dict[str, Any], articles: List[Dict[str, Any]], 
                                    schedule_options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            delivery_options = {
                ...
                'rss_url': options.get('rss_url', f"{self.config.podcast.rss_base_url}/podcast/feed.xml"),
                'image_urls': image_urls
            }
    ...
    def send_multiple_episodes(self, episodes: List[Dict[str, Any]], 
        ...
        try:
            ...
            # カルーセル形式で送信するか判定
            use_carousel = (
                options.get('use_carousel', True) and 
                len(episodes) > 1 and 
                len(episodes) <= 10 and
                options.get('use_flex_message', True)
            )
            ...
            self.logger.info(f"複数エピソード送信完了: 成功率 {success_count if 'success_count' in locals() else int(success)}/{len(episodes)}")
            return result
    ...
    def get_notification_preview(self, episode_info: Dict[str, Any], articles: List[Dict[str, Any]], 
                               preview_options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        通知のプレビューを生成
