...
class ImageAssetManager:
    ...
    def get_or_create_podcast_image(self, episode_info: Dict[str, Any], image_type: str = 'thumbnail') -> Optional[str]:
        """
        ポッドキャスト用画像を取得または生成
    ...
    def _generate_image_with_pillow(self, episode_info: Dict[str, Any], image_type: str) -> Optional[str]:
        """
        Pillowを使用して画像を生成
    ...
    def _draw_thumbnail(self, draw, width: int, height: int, date_str: str, article_count: int) -> None:
        """サムネイル画像の描画"""
        # 背景グラデーション（簡易版）
        ...
        draw.text(((width - articles_width) // 2, height - 40), articles_text, fill='white', font=font_small)
    
    def _draw_header(self, draw, width: int, height: int, date_str: str, article_count: int) -> None:
        """ヘッダー画像の描画"""
        # より大きなヘッダー用の描画
        # グラデーション背景
        for i in range(height):
        ...
        title_width = title_bbox[2] - title_bbox[0]
        draw.text(((width - title_width) // 2, height // 2 - 60), main_title, fill='white', font=font_title)
        
        # サブタイトル
        subtitle = f"AIが読み上げる最新マーケット情報 | {date_str}"
        subtitle_bbox = draw.textbbox((0, 0), subtitle, font=font_subtitle)
        subtitle_width = subtitle_bbox[2] - subtitle_bbox[0]
        draw.text(((width - subtitle_width) // 2, height // 2 + 20), subtitle, fill='white', font=font_subtitle)
    ...
    def _generate_cache_key(self, episode_info: Dict[str, Any], image_type: str) -> str:
        ...
        """
        key_data = {
            'date': episode_info.get('published_at', '').strftime('%Y-%m-%d') if isinstance(episode_info.get('published_at'), datetime) else str(episode_info.get('published_at', '')),
            'article_count': episode_info.get('article_count', 0),
            'type': image_type
        }
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_size_mb': sum(os.path.getsize(f) for f in self.assets_dir.glob('*.png')) / 1024 / 1024
        }