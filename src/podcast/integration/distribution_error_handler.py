...
class DistributionErrorHandler:
    ...
    def handle_error(self, 
        ...
        # ログ記録
        self.logger.error(f"Distribution error: {error_type.value} - {message}", 
    ...
    async def retry_operation(self, 
        ...
        if now < error_info.next_retry_time:
            ...
            self.logger.info(f"Waiting {wait_seconds:.1f}s before retry for error {error_info.error_id}")
        ...
        try:
            ...
            self.logger.info(f"Retry successful for error {error_info.error_id} (attempt {error_info.retry_count})")
            ...
            # 失敗時の処理
            self.logger.error(f"Retry failed for error {error_info.error_id} (attempt {error_info.retry_count}): {str(e)}")
            self.save_error_history()
    ...
    async def _execute_fallback_strategy(self, error_info: ErrorInfo):
        ...
        if fallback_handler:
            try:
                ...
                self.logger.error(f"Fallback strategy failed for {error_info.error_type.value}: {str(e)}")
        else:
            # デフォルト代替手段
    ...
    async def _default_fallback_strategy(self, error_info: ErrorInfo):
        """デフォルト代替手段"""
        if error_info.error_type in [ErrorType.TTS_GENERATION_FAILED, ErrorType.AUDIO_PROCESSING_FAILED]:
...
ご迷惑をおかけして申し訳ございません。"""
    ...
    def _notify_user_about_error(self, error_info: ErrorInfo):
        ...
        error_data = {
            ...
            "retry_count": error_info.retry_count,
            "next_retry": error_info.next_retry_time.strftime('%Y-%m-%d %H:%M:%S') if error_info.can_retry else ""
        }
...
システムは正常に動作しています。"""
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        if resolved_errors:
            total_resolution_time = sum([e.retry_count * e.backoff_seconds for e in resolved_errors])
    ...
    def load_error_history(self):
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(self.error_history)} error records from {self.error_log_path}")
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        success = await error_handler.retry_operation(
            error,
            ErrorHandlerTestHelper.mock_successful_operation
        )
