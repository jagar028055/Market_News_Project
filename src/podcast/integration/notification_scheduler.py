...
class NotificationScheduler:
    ...
    def schedule_podcast_notification(self, episode_info: Dict[str, Any], articles: List[Dict[str, Any]], 
                                    ...
                                    delivery_options: Optional[Dict[str, Any]] = None) -> str:
        """
        ポッドキャスト通知をスケジュール
        ...
        try:
            ...
            self.logger.info(f"通知スケジュール追加: {notification_id} (配信予定: {scheduled_time.strftime('%Y-%m-%d %H:%M:%S')})")
            ...
            try:
                ...
                if status == NotificationStatus.PENDING and scheduled_time <= current_time:
                    ...
                    if self._check_rate_limit():
                        ...
                        self.logger.warning(f"レート制限により配信延期: {notification['id']} -> {new_time}")
    ...
    def _handle_notification_failure(self, notification: Dict[str, Any]) -> None:
        ...
        if retry_count < self.max_retries:
            # リトライスケジュール
            retry_delay = self.retry_intervals[retry_count] if retry_count < len(self.retry_intervals) else self.retry_intervals[-1]
            ...
            self.logger.info(f"通知リトライスケジュール: {notification['id']} (試行 {retry_count + 1}/{self.max_retries}, {retry_delay}秒後)")
    ...
    def _calculate_optimal_time(self, priority: NotificationPriority) -> datetime:
        ...
        if priority == NotificationPriority.URGENT:
            ...
            return next_next_optimal if priority == NotificationPriority.NORMAL else next_next_optimal + timedelta(hours=1)
    ...
    def cancel_notification(self, notification_id: str) -> bool:
        ...
        for notification in self.notification_queue:
            if notification['id'] == notification_id:
                if notification['status'] in [NotificationStatus.PENDING.value, NotificationStatus.SCHEDULED.value]:
                    ...
                    self.logger.warning(f"通知キャンセル不可（ステータス: {notification['status']}）: {notification_id}")
                    return False
