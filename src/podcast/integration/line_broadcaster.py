...
class LineBroadcaster:
    ...
    def broadcast_podcast_notification(self, episode_info: Dict[str, Any], articles: List[Dict[str, Any]], audio_url: Optional[str] = None) -> bool:
        """
    ...
    def _create_podcast_message(self, episode_info: Dict[str, Any], articles: List[Dict[str, Any]], audio_url: Optional[str] = None) -> str:
        """
        ポッドキャスト通知メッセージを作成
        ...
        if audio_url:
            ...
            message_parts.extend([
                "🎧 AIが生成した高品質ポッドキャストをお楽しみください！",
                ""
            ])
        
        # ハッシュタグ（テストモード時は専用タグを追加）
        if test_mode:
            message_parts.append("#テスト配信 #GoogleCloudTTS #マーケットニュース #ポッドキャスト #AI")
        else:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning(f"LINE API エラー (試行 {attempt + 1}): ステータス {response.status_code}, レスポンス: {response.text}")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"LINE Bot接続テスト成功: {bot_info.get('displayName', 'Unknown')}")
                return True
            else:
