...
class ProductionPodcastIntegrationManager:
    ...
    def __init__(self, config: Config):
        ...
        self.script_generator = ProfessionalDialogueScriptGenerator(
            api_key=config.gemini.api_key, model_name=gemini_model
        )
    ...
    def generate_complete_podcast(
        self, hours_lookback: int = 24, max_articles: int = 6, test_mode: Optional[bool] = None
    ) -> Dict[str, Any]:
        """
        完全版ポッドキャスト生成・配信
        ...
        try:
            # Phase 1: 高品質記事選択
            selected_articles = self._select_high_quality_articles(hours_lookback, max_articles)
            ...
            # Phase 3: 高品質音声生成
            audio_result = self._generate_professional_audio(script_content, actual_test_mode)

            # Phase 4: GitHub Pages配信
            publish_result = self._publish_to_github_pages(audio_result, actual_test_mode)
    ...
    def _select_high_quality_articles(self, hours: int, max_articles: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            log_with_context(
                ...
                avg_importance=(
                    sum(a.get("importance_score", 0) for a in validated_articles)
    ...
    def _calculate_script_quality_score(self, script: str, articles: List[Dict[str, Any]]) -> float:
        ...
        article_titles_mentioned = sum(
            1 for article in articles if article.get("title", "")[:20] in script
        )
    ...
    def _publish_to_github_pages(
        self, audio_result: Dict[str, Any], test_mode: bool
    ) -> Dict[str, Any]:
        """
        GitHub Pages配信
        ...
        try:
            publish_result = self.github_publisher.publish_podcast_file(
                local_audio_path=audio_result["file_path"], test_mode=test_mode
            )
    ...
    def _broadcast_to_line(
        self, publish_result: Dict[str, Any], articles: List[Dict[str, Any]], test_mode: bool
    ) -> Dict[str, Any]:
        """
        LINE配信
        ...
        try:
            # メッセージ内容構築
            message_content = self._build_line_message_content(publish_result, articles, test_mode)
    ...
    def _build_line_message_content(
        ...
        for i, article in enumerate(articles[:4], 1):
            ...
            region_flag = {"usa": "🇺🇸", "japan": "🇯🇵", "china": "🇨🇳", "europe": "🇪🇺"}.get(
                region, "🌍"
            )
            title_short = (
        ...
        # 最終組み立て
        message_parts = [header, time_info, audio_info, "", article_summary, footer]
    ...
    def _compile_complete_result(
        ...
        result = {
            ...
            "quality_metrics": {
                ...
                "article_importance_avg": (
                    sum(a.get("importance_score", 0) for a in articles) / len(articles)
                    if articles
                    else 0
                ),
