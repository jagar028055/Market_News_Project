...
class EngagementAnalyzer:
    ...
    def analyze_user_engagement(self, user_id: str, days: int = 30) -> Optional[UserEngagementProfile]:
        """ユーザーエンゲージメント分析"""
        try:
            with sqlite3.connect(self.analytics.db_path) as conn:
                ...
                subscription_status = conn.execute("""
                    ...
                    AND event_time >= datetime('now', '-{} days')
                """.format(days), (user_id,)).fetchone()[0]
    ...
    def analyze_episode_engagement(self, episode_id: str) -> Optional[EpisodeEngagementSummary]:
        ...
        try:
            with sqlite3.connect(self.analytics.db_path) as conn:
                ...
                # 読取時間統計
                read_time = conn.execute("""
                    SELECT AVG(CAST(json_extract(event_data, '$.duration') AS REAL))
                    FROM engagement_events 
                    WHERE episode_id = ? AND event_type = 'read_time'
                """, (episode_id,)).fetchone()[0] or 0.0
    ...
    def _calculate_user_engagement_score(self, user_id: str, interactions: int, episodes: int, days: int) -> float:
        """ユーザーエンゲージメントスコア計算"""
        try:
            with sqlite3.connect(self.analytics.db_path) as conn:
                # 様々な行動の重み付け
                ...
                for action, weight in action_weights.items():
                    count = conn.execute("""
                        ...
                        AND event_time >= datetime('now', '-{} days')
                    """.format(days), (user_id, action)).fetchone()[0]
    ...
    def _calculate_retention_rate(self, episode_id: str) -> float:
        ...
        try:
            with sqlite3.connect(self.analytics.db_path) as conn:
                ...
                """, (episode_id, episode_id)).fetchone()[0] or 0
