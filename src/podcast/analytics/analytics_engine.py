...
class AnalyticsEngine:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # インデックス作成
            conn.execute("CREATE INDEX IF NOT EXISTS idx_delivery_episode ON delivery_events(episode_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_delivery_time ON delivery_events(delivery_time)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_engagement_episode ON engagement_events(episode_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_engagement_user ON engagement_events(user_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_performance_episode ON performance_metrics(episode_id)")
    ...
    def calculate_performance_metrics(self, episode_id: str, delivery_time: datetime.datetime) -> PerformanceMetrics:
        """パフォーマンス指標計算"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                # 配信統計取得
                ...
                """, (episode_id, delivery_time.isoformat())).fetchone()
                ...
                # 指標計算
                click_through_rate = (clicks / successful_deliveries) if successful_deliveries > 0 else 0.0
                engagement_rate = (unique_users / successful_deliveries) if successful_deliveries > 0 else 0.0
                conversion_rate = (conversions / successful_deliveries) if successful_deliveries > 0 else 0.0
