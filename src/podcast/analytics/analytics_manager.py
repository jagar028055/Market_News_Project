...
class AnalyticsManager:
    ...
    def record_delivery_success(
        self, tracking_id: str, response_time_ms: float, actual_count: Optional[int] = None
    ):
        """配信成功記録"""
    ...
    def record_delivery_failure(
        self, tracking_id: str, error_message: str, failed_count: Optional[int] = None
    ):
        """配信失敗記録"""
    ...
    # === パフォーマンス分析 ===
    def calculate_episode_performance(
            ...
            results.append(
                {
                    ...
                    "engagement_score": (
                        engagement_summary.engagement_score if engagement_summary else 0
                    ),
                }
            )
    ...
    def create_ab_test(
        ...
        return self.ab_tests.create_test(
            test_name, test_type_enum, variants, description, duration_days
        )
    ...
    def cleanup_old_data(self, days_to_keep: int = 90):
        ...
        try:
            ...
            self.logger.info(
                f"データクリーンアップ完了: エンゲージメント{deleted_engagement}件、配信{deleted_delivery}件削除"
            )
