...
class DashboardGenerator:
    ...
    def _generate_top_content_data(self, days: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'content_types': self._analyze_content_types(days)
        }
    ...
    def _generate_system_health_data(self) -> Dict[str, Any]:
        ...
        if total_events > 100000:
            issues.append('データベースサイズが大きくなっています。クリーンアップを検討してください')
    ...
    def _generate_charts(self, data: Dict[str, Any], days: int) -> Dict[str, str]:
        ...
        try:
            # パフォーマンストレンドチャート
            charts['performance_trend'] = self._create_performance_trend_chart(data['performance_trends'])
            
            # エンゲージメント分析チャート
            charts['engagement_heatmap'] = self._create_engagement_heatmap_chart(data['engagement_analysis'])
            
            # 配信成功率チャート
            charts['delivery_metrics'] = self._create_delivery_metrics_chart(data['performance_trends'])
    ...
    def _create_performance_trend_chart(self, trends_data: List[Dict[str, Any]]) -> str:
        ...
        # エンゲージメント率とクリック率
        ax1.plot(dates, [t['engagement_rate'] for t in trends_data], 'b-', label='エンゲージメント率', marker='o')
        ax1.plot(dates, [t['click_through_rate'] for t in trends_data], 'r-', label='クリック率', marker='s')
        ax1.set_ylabel('率 (%)')
        ...
        # 配信数と成功数
        ax2.bar(dates, [t['recipients'] for t in trends_data], alpha=0.7, label='配信数', color='skyblue')
        ax2.bar(dates, [t['delivered'] for t in trends_data], alpha=0.7, label='成功配信数', color='lightgreen')
        ax2.set_ylabel('件数')
        ax2.set_title('配信統計')
        ax2.legend()
    ...
    def _create_delivery_metrics_chart(self, trends_data: List[Dict[str, Any]]) -> str:
        ...
        # データ準備
        dates = [datetime.datetime.fromisoformat(t['date']) for t in trends_data]
        delivery_rates = [(t['delivered']/t['recipients'])*100 if t['recipients'] > 0 else 0 for t in trends_data]
    ...
    def _calculate_period_comparison(self, days: int) -> Dict[str, Any]:
        ...
        # 平均値計算
        current_avg_engagement = sum(t.engagement_rate for t in current_trends) / len(current_trends)
        previous_avg_engagement = sum(t.engagement_rate for t in previous_period) / len(previous_period)
        ...
        return {
            ...
            'engagement_rate_change_percent': round(
                ((current_avg_engagement - previous_avg_engagement) / previous_avg_engagement) * 100, 1
            ) if previous_avg_engagement > 0 else 0,
            'click_through_rate_change_percent': round(
                ((current_avg_ctr - previous_avg_ctr) / previous_avg_ctr) * 100, 1
            ) if previous_avg_ctr > 0 else 0
        }
