...
class ABTestManager:
    ...
    def _init_test_database(self):
        ...
        with sqlite3.connect(self.analytics.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_test_participants ON test_participants(test_id)")
        ...
        try:
            ...
            with sqlite3.connect(self.analytics.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    test_id, test_name, test_type.value, description,
                    start_time.isoformat(), end_time.isoformat(),
                    TestStatus.DRAFT.value, success_metric, minimum_sample_size,
                ...
                for variant in test_variants:
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """, (
                        variant.variant_id, test_id, variant.variant_name,
                        json.dumps(variant.variant_config), variant.allocated_percentage
                    ))
        ...
        with sqlite3.connect(self.analytics.db_path) as conn:
            ...
            for variant_id, variant_name in variants:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    result_id, test_id, variant_id, participant_count,
                    engaged_users, success_rate, avg_engagement_score
                ))
