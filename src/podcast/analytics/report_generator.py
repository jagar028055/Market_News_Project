...
"""
        ...
        return {
            ...
            "summary": {
                ...
                "avg_engagement_score": (
                    statistics.mean(t.avg_engagement_score for t in trends) if trends else 0
                ),
            },
        }
    ...
    def _get_ab_test_summary(self) -> Dict[str, Any]:
        ...
        return {"active_count": len(active_tests), "tests": active_tests}
    ...
    def _get_growth_metrics(self, days: int) -> Dict[str, Any]:
        ...
        current_avg_engagement = (
            statistics.mean(t.engagement_rate for t in current_trends) if current_trends else 0
        )
        previous_avg_engagement = (
            statistics.mean(t.engagement_rate for t in previous_period) if previous_period else 0
        )

        growth_rate = (
            ((current_avg_engagement - previous_avg_engagement) / previous_avg_engagement * 100)
    ...
    def _generate_recommendations(self, days: int) -> List[Dict[str, Any]]:
        ...
        if trends:
            ...
            # 配信失敗率チェック
            avg_delivery_rate = statistics.mean(
                (t.successful_deliveries / t.total_recipients * 100)
                for t in trends
                if t.total_recipients > 0
            )
    ...
    def _analyze_ab_test_results(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_level": (
                "high" if winner.get("statistical_significance", 0.5) <= 0.05 else "low"
            ),
        }
