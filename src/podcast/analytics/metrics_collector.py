...
class MetricsCollector:
    ...
    def start_notification_tracking(
        self, episode_id: str, message_type: str, recipient_count: int
    ) -> str:
        """通知トラッキング開始"""
    ...
    def record_delivery_failure(
        self, notification_id: str, error_message: str, failed_count: Optional[int] = None
    ):
        """配信失敗記録"""
        if notification_id not in self._active_notifications:
            self.logger.warning(f"不明な通知ID: {notification_id}")
    ...
    def record_user_click(
        ...
        event = EngagementEvent(
            ...
            event_data={"click_target": click_target, **(additional_data or {})},
        )
    ...
    def record_user_view(
        self, episode_id: str, user_id: str, view_duration_ms: Optional[int] = None
    ):
        """ユーザービュー記録"""
        event = EngagementEvent(
            event_id=str(uuid.uuid4()),
            ...
            event_type="view",
            event_time=datetime.datetime.now(),
            event_data={"duration_ms": view_duration_ms} if view_duration_ms else {},
        )
    ...
    def record_user_share(self, episode_id: str, user_id: str, share_platform: str):
        ...
        event = EngagementEvent(
            ...
            event_type="share",
            event_time=datetime.datetime.now(),
            event_data={"platform": share_platform},
        )
    ...
    def record_subscription(self, episode_id: str, user_id: str, subscription_type: str = "rss"):
        ...
        event = EngagementEvent(
            ...
            event_time=datetime.datetime.now(),
            event_data={"subscription_type": subscription_type},
        )

        self.analytics.record_engagement_event(event)
        self.logger.info(f"購読記録: {user_id} ({subscription_type})")

    def record_read_time(self, episode_id: str, user_id: str, read_duration_seconds: float):
        """読取時間記録"""
        event = EngagementEvent(
            event_id=str(uuid.uuid4()),
            episode_id=episode_id,
            ...
            event_time=datetime.datetime.now(),
            event_data={"duration": read_duration_seconds},
        )
    ...
    def calculate_realtime_stats(self) -> Dict[str, Any]:
        ...
        if active_count == 0:
            return {"active_notifications": 0, "total_recipients": 0, "avg_response_time": 0.0}

        total_recipients = sum(m.recipient_count for m in self._active_notifications.values())
        completed_notifications = [
            m
            for m in self._active_notifications.values()
            if m.success_count > 0 or m.failure_count > 0
        ]

        avg_response_time = 0.0
        if completed_notifications:
            avg_response_time = sum(m.response_time_ms for m in completed_notifications) / len(
                completed_notifications
            )
