...
class GeminiTTSEngine:
    ...
    def _synthesize_segment(self, segment: SpeakerSegment) -> bytes:
        ...
        try:
            ...
            self.logger.debug(f"セグメント合成完了: speaker{segment.speaker_id} ({voice_name}), {len(segment.text)}文字, {duration_ms}ms")
            return audio_data
        ...
        try:
            ...
            for i, segment in enumerate(segments):
                if segment.audio_data:
                    ...
                    self.logger.debug(f"セグメント結合: {i+1}/{total_segments}, speaker{segment.speaker_id}")
            ...
            self.logger.info(f"音声結合完了: 総再生時間 {total_duration_sec:.2f}秒, {total_segments}セグメント")
