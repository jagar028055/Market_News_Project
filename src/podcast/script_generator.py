...
class DialogueScriptGenerator:
    ...
    def _prioritize_articles(self, articles: List[Dict]) -> List[ArticlePriority]:
        ...
        for article in articles:
            ...
            prioritized.append(ArticlePriority(
    ...
    def _calculate_priority(self, article: Dict) -> float:
        ...
        high_priority_keywords = [
            '日銀', '金利', 'fed', 'fomc', '株価', '円安', '円高',
            'gdp', 'cpi', 'インフレ', 'デフレ', '決算', '業績'
        ]
    ...
    def _categorize_article(self, article: Dict) -> str:
        ...
        if any(keyword in title or keyword in content for keyword in ['日銀', 'fed', '金利', '政策']):
            return '金融政策'
        elif any(keyword in title or keyword in content for keyword in ['株価', '日経', 'dow', 'nasdaq']):
            return '株式市場'
        elif any(keyword in title or keyword in content for keyword in ['円安', '円高', 'ドル', 'ユーロ']):
            return '為替'
        elif any(keyword in title or keyword in content for keyword in ['決算', '業績', '売上', '利益']):
            return '企業業績'
        else:
            return '一般経済'
...
"""
    ...
    def _format_dialogue(self, raw_script: str) -> str:
        ...
        for line in lines:
            ...
            if cleaned_line:
                ...
                formatted_lines.append(f'<speaker{current_speaker}>{speaker_name}</speaker{current_speaker}>:{cleaned_line}')
    ...
    def _trim_script(self, script: str) -> str:
        ...
        for line in lines:
            if any(keyword in line.lower() for keyword in ['おはようございます', 'こんにちは', 'ありがとう', 'また明日']):
