...
from src.podcast.integration.production_podcast_integration_manager import (
    ProductionPodcastIntegrationManager,
)
...
def setup_podcast_logging(mode: str) -> logging.Logger:
    ...
    # ログ設定
    setup_logging(level=logging.INFO, log_file=str(log_file), format_type="detailed")
...
async def run_podcast_production(
    mode: str, duration: int, config_overrides: Optional[dict] = None
) -> bool:
    """
    ポッドキャスト制作・配信実行
    ...
    try:
        ...
        result = await integration_manager.execute_full_pipeline(
            target_duration=duration, test_mode=(mode == "test")
        )
...
使用例:
  ...
  python -m src.podcast.production_main --mode=test --duration=300 --script-model=gemini-2.5-flash-lite
    ...
    parser.add_argument(
        "--mode", choices=["test", "production"], default="test", help="実行モード (default: test)"
    )

    parser.add_argument(
        "--duration", type=int, default=600, help="ポッドキャスト目標時間（秒） (default: 600)"
    )

    parser.add_argument("--script-model", type=str, help="台本生成モデル上書き")

    parser.add_argument("--script-temperature", type=float, help="台本生成温度上書き")

    parser.add_argument("--no-line-notify", action="store_true", help="LINE通知を無効化")
    ...
    try:
        success = asyncio.run(
            run_podcast_production(
                mode=args.mode, duration=args.duration, config_overrides=config_overrides
            )
        )
