    ...
    def publish(self, episode: PodcastEpisode, credits: str) -> PublishResult:
        ...
        try:
            ...
            return PublishResult(
                channel="RSS", success=True, message="RSS配信が正常に完了しました", url=rss_url
            )

        except Exception as e:
            self.logger.error(f"RSS配信に失敗: {str(e)}")
            return PublishResult(channel="RSS", success=False, message=f"RSS配信エラー: {str(e)}")
...
class LINEBroadcaster:
    ...
    def publish(self, episode: PodcastEpisode, podcast_url: str, credits: str) -> PublishResult:
        ...
        try:
            ...
            return PublishResult(
                channel="LINE", success=True, message=message_text, url=podcast_url
            )

        except Exception as e:
            self.logger.error(f"LINE配信に失敗: {str(e)}")
            return PublishResult(channel="LINE", success=False, message=f"LINE配信エラー: {str(e)}")

    def _create_broadcast_message(
        self, episode: PodcastEpisode, podcast_url: str, credits: str
    ) -> Dict[str, Any]:
        """
        ブロードキャストメッセージを作成
...
{credits}"""
        ...
        if len(message_text) > self.max_message_length:
            ...
            max_description_length = (
                self.max_message_length - len(message_text) + len(episode.description) - 100
            )
...
{credits}"""
        ...
        return {"type": "text", "text": message_text.strip()}
    ...
    def _validate_message_length(self, message: Dict[str, Any]) -> None:
        ...
        if len(text) > self.max_message_length:
            raise LINEBroadcastingError(
                f"メッセージが長すぎます: {len(text)} > {self.max_message_length}"
            )
    ...
    def _send_broadcast_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        payload = {"messages": [message], "notificationDisabled": not self.enable_notification}

        return self._make_api_request(self.broadcast_endpoint, headers, payload)

    def _send_multicast_message(
        self, message: Dict[str, Any], user_ids: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def _make_api_request(
        ...
        for attempt in range(self.retry_count):
            try:
                self.logger.debug(
                    f"LINE API呼び出し (試行 {attempt + 1}/{self.retry_count}): {url}"
                )
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"LINE API接続テスト成功: {bot_info.get('displayName', 'Unknown Bot')}"
            )
            return True
        except Exception as e:
            ...
            "contents": {
                ...
                "body": {
                    ...
                    "contents": [
                        ...
                        {"type": "text", "text": episode.description, "wrap": True, "margin": "md"},
                    ],
                },
                "footer": {
                    "type": "box",
                    ...
                    "contents": [
                        {
                            "type": "button",
                            "style": "primary",
                            "action": {"type": "uri", "label": "🎧 聴く", "uri": podcast_url},
                        },
                        {
                            "type": "text",
                            "text": credits,
    ...
    def publish(self, episode: PodcastEpisode, credits: str) -> List[PublishResult]:
        ...
        try:
            ...
            if rss_result.success and rss_result.url:
                ...
                results.append(
                    PublishResult(
                        channel="LINE", success=False, message="RSS配信失敗のためLINE配信をスキップ"
                    )
                )
            ...
            results.append(
                PublishResult(channel="RSS", success=False, message=f"RSS配信エラー: {str(e)}")
            )
            results.append(
                PublishResult(
                    channel="LINE", success=False, message="RSS配信失敗のためLINE配信をスキップ"
