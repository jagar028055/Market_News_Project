...
class PublishErrorHandler:
    ...
    def execute_with_retry(
        self, func: Callable, episode_guid: str, channel: str, *args, **kwargs
    ) -> bool:
        """再試行機能付きで関数を実行
        ...
        for attempt in range(self.retry_strategy.max_retries + 1):
            try:
                ...
                if not self.retry_strategy.should_retry(error_type, attempt):
                    logger.error(
                        f"{channel} publish failed after {attempt + 1} attempts: {last_error}"
                    )
                    return False
                ...
                if attempt < self.retry_strategy.max_retries:
                    ...
                    logger.info(
                        f"{channel} publish failed (attempt {attempt + 1}), retrying in {delay:.1f}s..."
                    )
                    time.sleep(delay)
    ...
    def _classify_error_type(self, error: Optional[Exception], channel: str) -> PublishErrorType:
        ...
        if error is None:
            return (
                ...
                if channel == "rss"
                else PublishErrorType.LINE_API_ERROR
            )
