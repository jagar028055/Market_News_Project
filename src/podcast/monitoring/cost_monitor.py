...
class CostMonitor:
    ...
    def track_tts_usage(self, character_count: int, model_name: str = "gemini-tts",
                       episode_id: Optional[str] = None, 
    ...
    def get_current_month_costs(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {'total': 0, 'characters': 0, 'records_count': 0, 'remaining_budget': self.monthly_limit}
    ...
    def _check_cost_warnings(self) -> None:
        ...
        try:
            ...
            if monthly_costs['total'] >= self.monthly_limit * 0.9:
                logger.warning(f"Cost approaching monthly limit: ${monthly_costs['total']:.2f} / ${self.monthly_limit:.2f}")
                
            # 月末予想コストが制限を超える場合
            if cost_summary.projected_month_end_cost > self.monthly_limit:
                logger.warning(f"Projected month-end cost exceeds limit: ${cost_summary.projected_month_end_cost:.2f}")
    ...
    def export_usage_report(self, month: Optional[int] = None, year: Optional[int] = None) -> Dict:
        ...
        try:
            ...
            if report['episodes_processed'] > 0:
                report['average_cost_per_episode'] = report['total_cost'] / report['episodes_processed']
