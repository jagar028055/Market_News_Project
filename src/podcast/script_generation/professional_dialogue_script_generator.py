...
"""
    ...
    def _build_professional_prompt(
        self, articles: List[Dict[str, Any]], analysis: Dict[str, Any]
    ) -> str:
        """
        プロ版プロンプト構築
        ...
        category_dist_str = ", ".join(
            [f"{k}: {v}件" for k, v in analysis["category_distribution"].items()]
        )
        region_dist_str = ", ".join(
            [f"{k}: {v}件" for k, v in analysis["region_distribution"].items()]
        )
    ...
    def _format_articles_for_professional_analysis(self, articles: List[Dict[str, Any]]) -> str:
        ...
        for i, article in enumerate(articles, 1):
            ...
            importance_rank = (
                "🔴 最重要"
                if article.get("importance_score", 0) >= 8
                else (
                    "🟠 重要"
                    if article.get("importance_score", 0) >= 6
                    else "🟡 注目" if article.get("importance_score", 0) >= 4 else "🔵 補完"
                )
            )
...
"""
    ...
    def _generate_with_professional_settings(self, prompt: str) -> str:
        ...
        try:
            ...
            if self.is_pro_model:
                ...
                # 既存モデル用設定
                generation_config = genai.types.GenerationConfig(
                    max_output_tokens=3000, temperature=0.3, top_p=0.8, top_k=40
                )
    ...
    def _expand_script_professionally(
        self, script: str, articles: List[Dict[str, Any]], needed_chars: int
    ) -> str:
        """プロ品質での台本拡張"""
        # より詳細な市場分析を追加
        expansion_content = "\\n\\n【補足市場分析】\\n"
        ...
        expansion_content += (
            "これらの動向は今後数週間にわたって市場参加者の注目を集め続けると予想されます。"
        )
