...
class GeminiTTSEngine:
    ...
    def _preprocess_pronunciation(self, script: str) -> str:
        ...
        # 【NEW】TTS直前の最終クリーニング
        processed = self._final_tts_cleanup(processed)
    ...
    def _final_tts_cleanup(self, text: str) -> str:
        """
        TTS直前の最終クリーニング処理
        ...
        for pattern in markdown_cleanup:
            if '(' in pattern:  # キャプチャグループありの場合
        ...
        if len(cleaned) != len(text):
            self.logger.info(f"TTS最終クリーニング: {len(text)} → {len(cleaned)}文字 ({len(text)-len(cleaned)}文字除去)")
