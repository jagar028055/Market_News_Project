...
class AudioProcessor:
    ...
    def process_audio(self, tts_audio: bytes) -> AudioProcessingResult:
        ...
        try:
            ...
            self.logger.info(f"音声処理完了: {final_result.duration_seconds}秒, {final_result.file_size_bytes/1024/1024:.2f}MB")
            return final_result
    ...
    def _adjust_duration(self, audio: AudioSegment) -> AudioSegment:
        ...
        if current_duration_minutes > self.max_duration_minutes:
            ...
            self.logger.warning(f"音声を短縮: {current_duration_minutes:.2f} -> {len(audio)/1000/60:.2f}分")
    ...
    def _finalize_audio(self, audio: AudioSegment) -> AudioProcessingResult:
        ...
        if file_size_mb > self.max_file_size_mb:
            self.logger.warning(f"ファイルサイズが制限を超過: {file_size_mb:.2f}MB > {self.max_file_size_mb}MB")
            # より低いビットレートで再エクスポート
