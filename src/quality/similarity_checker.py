...
class SimilarityChecker:
    ...
    def check_similarity_batch(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for i in range(len(articles)):
            for j in range(i + 1, len(articles)):
                ...
                # より実用的な類似度判定（タイトルが完全一致または非常に高い類似度のみ）
                title_only_sim = self._cosine_similarity(
                    articles[i].get("title", ""), articles[j].get("title", "")
                )
                if similarity >= self.similarity_threshold and (
                    title_only_sim > 0.8 or similarity > 0.85
                ):
        ...
        recommendations = self._generate_recommendations(
            similar_pairs, duplicate_groups, diversity_score
        )
        ...
        self.logger.info(
            f"類似度チェック完了: 類似ペア={len(similar_pairs)}組, 多様性={diversity_score:.3f}"
        )
        return result
    ...
    def _calculate_similarity(self, article1: Dict[str, Any], article2: Dict[str, Any]) -> float:
        ...
        # 複数の類似度指標を組み合わせ
        title_sim = self._cosine_similarity(article1.get("title", ""), article2.get("title", ""))

        summary_sim = self._cosine_similarity(
            article1.get("summary", ""), article2.get("summary", "")
        )
        ...
        # 加重平均で最終類似度を計算
        final_similarity = (
            title_sim * 0.3  # タイトルの重み（軽減）
            + summary_sim * 0.6  # 要約の重み（増加）
    ...
    def _find_duplicate_groups(
        self, similarity_matrix: Dict[Tuple[int, int], float], num_articles: int
    ) -> List[List[int]]:
        """重複グループを特定"""
        ...
        for i in range(num_articles):
            ...
            def find_similar(article_idx):
                for j in range(num_articles):
                    ...
                    if (
                        pair in similarity_matrix
                        and similarity_matrix[pair] >= self.similarity_threshold
                    ):
                        group.append(j)
                        visited.add(j)
    ...
    def _calculate_diversity_score(
        self, articles: List[Dict[str, Any]], similarity_matrix: Dict[Tuple[int, int], float]
    ) -> float:
        """コンテンツ多様性スコアを計算"""
        if len(articles) < 2:
            return 1.0
