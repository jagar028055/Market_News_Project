...
class ContentValidator:
    ...
    def validate_article(self, article: Dict[str, Any]) -> ValidationResult:
        ...
        scores['information_completeness'] = self._check_information_completeness(article, issues, recommendations)
        ...
        total_score = sum(
            scores[item] * weight 
            for item, weight in self.quality_weights.items()
        )
        
        # 合格判定
        passed = total_score >= self.min_quality_score and len(issues) == 0
    ...
    def validate_article_batch(self, articles: List[Dict[str, Any]]) -> List[ValidationResult]:
        ...
        self.logger.info(f"バッチバリデーション完了: 合格={passed_count}/{len(articles)}, 平均スコア={avg_score:.1f}")
        return results
    
    def _check_summary_length(self, article: Dict[str, Any], issues: List[str], recommendations: List[str]) -> float:
        """要約文字数チェック"""
    ...
    def _check_summary_quality(self, article: Dict[str, Any], issues: List[str], recommendations: List[str]) -> float:
        """要約品質チェック"""
        summary = article.get('summary', '')
        if not summary:
            return 0.0
    ...
    def _check_information_completeness(self, article: Dict[str, Any], issues: List[str], recommendations: List[str]) -> float:
        """情報完全性チェック"""
        score = 100.0
    ...
    def _check_language_quality(self, article: Dict[str, Any], issues: List[str], recommendations: List[str]) -> float:
        """言語品質チェック"""
    ...
    def _check_consistency(self, article: Dict[str, Any], issues: List[str], recommendations: List[str]) -> float:
        """一貫性チェック"""
        score = 100.0
    ...
    def _check_relevance(self, article: Dict[str, Any], issues: List[str], recommendations: List[str]) -> float:
        """関連性チェック"""
        score = 100.0
        
        # 市場関連キーワードの存在チェック
        market_keywords = [
            '株価', '市場', '投資', '経済', '金融', '企業', '業績', '売上', '利益',
            '金利', 'GDP', 'インフレ', '為替', '通貨', '政策', '銀行', '証券'
        ]
    ...
    def _collect_details(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'sentence_count': len([s for s in re.split(r'[。．]', summary) if s.strip()]) if summary else 0,
    ...
    def get_quality_report(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        return {
            'summary': {
                ...
                'pass_rate': round(passed_articles / total_articles * 100, 1) if total_articles > 0 else 0
            },
            'score_statistics': {
