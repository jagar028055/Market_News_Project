...
class DynamicUserSegmentation:
    ...
    def analyze_user_behavior(
        self, user_id: str, interaction_history: List[Dict[str, Any]], time_window_days: int = 30
    ) -> Dict[str, float]:
        """ユーザー行動分析"""
        ...
        # セッション継続時間
        session_durations = [
            interaction.get("session_duration", 0) for interaction in recent_interactions
        ]
        ...
        # デバイス利用パターン
        device_types = [
            interaction.get("device_type", "unknown") for interaction in recent_interactions
        ]
    ...
    def _calculate_segment_match_score(
        self, user_features: Dict[str, float], segment: UserSegment
    ) -> float:
        """セグメント適合度スコア計算"""
    ...
    def assign_user_to_segments(self, user_id: str, segment_scores: Dict[str, float]) -> str:
        ...
        should_migrate = (
            ...
            or current_membership.segment_id != best_segment_id
            and best_score - segment_scores.get(current_membership.segment_id, 0)
    ...
    def _update_segment_membership(
        self,
        user_id: str,
        segment_id: str,
        score: float,
        confidence: float,
        previous_membership: Optional[UserSegmentMembership],
    ):
        """セグメント所属情報更新"""
    ...
    def _record_segment_migration(
        self, user_id: str, old_segment_id: str, new_segment_id: str, migration_score: float
    ):
        """セグメント移行記録"""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
    ...
    def _get_segment_by_id(self, segment_id: str) -> Optional[UserSegment]:
        """ID指定でセグメント取得"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT * FROM user_segments WHERE segment_id = ?", (segment_id,))
            row = cursor.fetchone()
    ...
    def calculate_segmentation_metrics(self) -> SegmentationMetrics:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            total_users = conn.execute(
                "SELECT COUNT(DISTINCT user_id) FROM segment_memberships"
            ).fetchone()[0]

            # 総セグメント数（アクティブ）
            total_segments = conn.execute(
                "SELECT COUNT(*) FROM user_segments WHERE user_count > 0"
            ).fetchone()[0]
        ...
        size_balance = (
            1.0 - np.std(segment_sizes) / max(avg_segment_size, 1) if segment_sizes else 0
        )
