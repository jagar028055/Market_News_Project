...
class IntelligentTimingOptimizer:
    ...
    def analyze_user_timing_patterns(
    ...
    def calculate_delivery_windows(
        self, user_id: str, timing_patterns: Dict[str, Any], channels: List[DeliveryChannel] = None
    ) -> List[DeliveryWindow]:
        """配信時間窓計算"""
    ...
    def optimize_delivery_timing(
        ...
        if strategy == TimingStrategy.IMMEDIATE:
            ...
            recommended_time = self._calculate_optimal_time_in_window(best_window, timing_patterns)
            confidence = self._calculate_timing_confidence(
                timing_patterns, len(interaction_history)
            )

        # フォールバック時間
        fallback_times = self._generate_fallback_times(recommended_time, delivery_windows[1:4])
    ...
    def _determine_timing_strategy(
        self, timing_patterns: Dict[str, Any], urgency_level: str, data_points: int
    ) -> TimingStrategy:
        """タイミング戦略決定"""
    ...
    def _calculate_optimal_time_in_window(
        self, window: DeliveryWindow, timing_patterns: Dict[str, Any]
    ) -> datetime:
        """時間窓内の最適時間計算"""
        ...
        # 明日の該当時間
        target_time = datetime.now().replace(hour=best_hour, minute=0, second=0, microsecond=0)
    ...
    def _calculate_timing_confidence(
        self, timing_patterns: Dict[str, Any], data_points: int
    ) -> float:
        """タイミング信頼度計算"""
    ...
    def _generate_fallback_times(
        self, primary_time: datetime, alternative_windows: List[DeliveryWindow]
    ) -> List[datetime]:
        """フォールバック時間生成"""
        ...
        for window in alternative_windows:
            fallback_time = datetime.now().replace(
                hour=window.start_time.hour, minute=0, second=0, microsecond=0
            )
    ...
    def _generate_timing_reasoning(
        ...
        strategy: TimingStrategy,
        best_window: Optional[DeliveryWindow],
    ) -> Dict[str, Any]:
        """タイミング推論理由生成"""

        reasoning = {"strategy": strategy.value, "factors": {}}
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    ...
                    (
                        performance.engagement_time.isoformat()
                        if performance.engagement_time
                        else None
                    ),
        ...
        if existing:
            ...
            if performance.engagement_time and performance.actual_delivery_time:
                response_time_minutes = int(
                    (performance.engagement_time - performance.actual_delivery_time).total_seconds()
                    / 60
                )
            ...
            new_response_rate = (existing[1] * old_sample_size + response_rate) / new_sample_size
