...
class UserOptimizer:
    ...
    def record_user_interaction(
        self, user_id: str, content_id: str, action_type: str, session_duration: int = 0
    ):
        """ユーザー行動記録"""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
    ...
    def _update_engagement_score(self, user_id: str, action_type: str, session_duration: int):
        ...
        # アクション種別による重み
        action_weights = {"view": 0.1, "click": 0.3, "share": 0.5, "save": 0.4, "comment": 0.6}
    ...
    def optimize_content_for_user(
        self, user_id: str, available_content: List[Dict[str, Any]]
    ...
    def _calculate_content_score(self, profile: UserProfile, content: Dict[str, Any]) -> float:
        ...
        if profile.reading_time_preference == "morning" and 6 <= current_hour <= 11:
            ...
            score *= 1.1
        elif profile.reading_time_preference == "night" and (
            current_hour >= 22 or current_hour <= 5
        ):
            score *= 1.1
    ...
    def get_user_analytics(self, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                actions[row[0]] = {"count": row[1], "avg_duration": row[2]}
...
if __name__ == "__main__":
    ...
    # テストユーザー作成
    test_user = optimizer.create_user_profile(
        "test_user_001",
        {
