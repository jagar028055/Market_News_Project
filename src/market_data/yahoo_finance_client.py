...
class YahooFinanceClient:
    ...
    def get_multiple_prices(self, symbols: List[str]) -> List[MarketData]:
        ...
        try:
            ...
            for i, symbol in enumerate(symbols):
                try:
                    ...
                    ticker = tickers.tickers[yf_symbol] if hasattr(tickers, 'tickers') else yf.Ticker(yf_symbol)
        ...
        self.logger.info(f"Successfully fetched {len(results)}/{len(symbols)} symbols from Yahoo Finance")
        return results
    ...
    def _determine_data_type_and_name(self, symbol: str, info: Dict[str, Any]) -> tuple[MarketDataType, str]:
        """
        ...
        # クォートタイプによる判定
        if quote_type in ['INDEX', 'EQUITY'] and (symbol.startswith('^') or 'index' in long_name.lower()):
    ...
    def _determine_region(self, symbol: str) -> str:
        """株価指数の地域を判定"""
        region_mapping = {
            '^GSPC': 'usa', '^DJI': 'usa', '^IXIC': 'usa', '^RUT': 'usa',
            '^N225': 'japan', '^NKX': 'japan',
            '^FTSE': 'europe', '^GDAXI': 'europe', '^FCHI': 'europe',
            '^HSI': 'china', '000001.SS': 'china',
        }
    ...
    def get_market_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'major_movers': [
                data.to_dict() for data in summary_data 
