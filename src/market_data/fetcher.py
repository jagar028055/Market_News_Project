...
class MarketDataFetcher:
    ...
    def get_current_market_snapshot(
        self, custom_symbols: Optional[List[str]] = None, use_cache: bool = True
    ) -> MarketSnapshot:
        """
        現在の市場スナップショットを取得
        ...
        self.logger.info(
            f"Market snapshot completed in {elapsed_time:.2f}s: "
    ...
    def _fetch_multiple_with_fallback(
    ...
    def _save_to_cache(self, symbol: str, data: Dict[str, Any]):
        """データをキャッシュに保存"""
        self._cache[symbol] = {"data": data, "cached_at": datetime.now().isoformat()}
        ...
        try:
            ...
            if snapshot.commodities:
                ...
                for commodity in snapshot.commodities[:3]:  # 上位3つ
                    direction = (
                        "↑" if commodity.change > 0 else "↓" if commodity.change < 0 else "→"
                    )
                    context_parts.append(
            ...
            if snapshot.major_movers:
                ...
                for mover in snapshot.major_movers[:3]:
                    direction = (
                        "急上昇"
                        if mover.change_percent > 2
                        else "急落" if mover.change_percent < -2 else "大幅変動"
                    )
                    context_parts.append(
