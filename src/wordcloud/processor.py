...
class TextProcessor:
        ...
        try:
            ...
            while node:
                ...
                if (word and 
                    ...
                    pos in self.config.pos_filter and
    ...
    def _simple_word_extraction(self, text: str) -> List[str]:
        ...
        for word in word_candidates:
            ...
            if (cleaned_word and
                len(cleaned_word) >= self.config.min_word_length and
                len(cleaned_word) <= self.config.max_word_length and
                cleaned_word not in self.stopwords and
                not self._is_numeric(cleaned_word)):
    ...
    def calculate_word_frequencies(self, words: List[str]) -> Dict[str, int]:
        ...
        filtered_frequencies = {
            word: freq for word, freq in frequency_counter.items()
    ...
    def process(self, articles: List[Dict]) -> Dict[str, int]:
        ...
        if self.config.use_tfidf:
            ...
            tfidf_scores = self.calculate_tfidf_scores(article_texts, list(weighted_frequencies.keys()))
