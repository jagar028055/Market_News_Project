...
class WordCloudGenerator:
    ...
    def generate_daily_wordcloud(self, articles: List[Dict]) -> WordCloudResult:
        ...
        try:
            ...
            if not word_frequencies:
                return WordCloudResult(
                    success=False,
                    error_message="有効な単語が抽出されませんでした"
                )
            ...
            if not image_result.success:
                return WordCloudResult(
                    success=False,
                    error_message=f"画像生成失敗: {image_result.error_message}"
                )
            ...
            quality_score = self._calculate_quality_score(
                word_frequencies, 
                len(articles),
                image_result.image_size_bytes or 0
            )
    ...
    def _calculate_quality_score(self, word_frequencies: Dict[str, int], 
                               article_count: int, image_size_bytes: int) -> float:
        """ワードクラウドの品質スコアを計算
        ...
        try:
            ...
            # 5. 金融用語の含有率（0-10点）
            financial_word_count = sum(1 for word in word_frequencies.keys() 
    ...
    def generate_wordcloud_for_session(self, session_id: int, articles: List[Dict]) -> WordCloudResult:
        """セッション用ワードクラウドを生成
