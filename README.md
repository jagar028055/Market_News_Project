...
- **複数サイトからのニュース収集**:
  ...
  - ブルームバーグ（日本語版）
- **AIによる自動要約**:
  - Googleの**Gemini API**を利用して、収集した各記事の本文を簡潔な要約に変換
- **Webページとしての自動公開**:
  - 要約されたニュース一覧をHTMLファイルとして生成し、**GitHub Pages**を利用して自動でWeb上に公開
- **柔軟なフィルタリング**:
  - 収集する記事の期間（例: 過去24時間以内）を指定可能
  - キーワード、カテゴリ、除外ワードを設定し、必要な情報だけを抽出
- **Googleドキュメントへの自動出力**:
  - **AI要約済み記事**: 日次アーカイブとして、指定したGoogle Driveフォルダに新しいドキュメントを作成
  - **収集した全記事（原文）**: 指定した単一のGoogleドキュメントに、常に最新の状態で上書き保存

### 🎙️ ポッドキャスト機能
- **音声ニュース配信**:
  - Google Cloud Text-to-Speech APIによる高品質な音声合成
  - 日次ポッドキャストの自動生成・配信
  - RSS 2.0形式でのポッドキャストフィード提供
- **LINE Bot統合**:
  - 配信完了通知の自動送信
  - カスタマイズ可能な通知メッセージ
- **コスト管理**:
