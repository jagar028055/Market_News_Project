...
class TestTTSEngine:
    ...
    def test_voice_configuration(self, mock_client_class):
        ...
        try:
            ...
            with patch.dict(
                ...
                # 合成メソッドが3回呼ばれることを確認
                assert (
                    mock_client.synthesize_speech.call_count == 3
                ), "音声合成の呼び出し回数が正しくない"
    ...
    def test_credentials_validation(self):
        ...
        try:
            ...
            # 不正な認証情報でのテスト
            invalid_credentials = {"type": "invalid_account"}  # 必須フィールドが不足

            with patch.dict(
                os.environ, {"GOOGLE_APPLICATION_CREDENTIALS_JSON": json.dumps(invalid_credentials)}
            ):
                # エラーハンドリングのテスト
                try:
                    engine = TTSEngine()
    ...
    def test_audio_config_settings(self, mock_client_class):
        ...
        try:
            ...
            with patch.dict(
                ...
                # カスタム音声設定でエンジンを作成
                engine = TTSEngine(speaking_rate=1.2, pitch=0.5, volume_gain_db=2.0)
    ...
    def test_cost_calculation(self):
        ...
        try:
            ...
            for text, expected_length in test_cases:
                ...
                ), f"文字数計算が正しくない: {actual_length} != {expected_length}"
