...
class TestDynamicArticleCollection(unittest.TestCase):
    ...
    def test_get_dynamic_hours_limit_monday(self):
        ...
        with patch("src.core.news_processor.datetime") as mock_datetime, patch(
            "src.core.news_processor.pytz"
        ) as mock_pytz:
            ...
            self.processor.logger.info.assert_called_with("月曜日検出: 自動的に72時間範囲を適用")
    ...
    def test_collect_articles_with_hours(self, mock_bloomberg, mock_reuters):
        ...
        mock_bloomberg.assert_called_once_with(
            hours_limit=48, exclude_keywords=self.processor.config.bloomberg.exclude_keywords
        )
...
class TestScrapingConfigExtension(unittest.TestCase):
    ...
    def test_scraping_config_custom_values(self):
        """カスタム値のテスト"""
        config = ScrapingConfig(
            hours_limit=12, minimum_article_count=50, max_hours_limit=96, weekend_hours_extension=60
        )
