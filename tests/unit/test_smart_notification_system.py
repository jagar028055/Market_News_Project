...
class TestFlexMessageTemplates(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_articles = [
            {"title": "日経平均株価が大幅上昇、年内最高値を更新", "sentiment_label": "Positive"},
            {"title": "米FRBの利上げ決定が市場に与える影響", "sentiment_label": "Neutral"},
            {"title": "暗号通貨市場で大幅な下落が続く", "sentiment_label": "Negative"},
        ]
    ...
    def test_create_article_highlights(self):
        """記事ハイライト作成テスト"""
        highlights = self.templates._create_article_highlights(
            self.sample_articles, max_highlights=2
        )
    ...
    def test_fallback_message(self):
        ...
        fallback = self.templates._create_simple_fallback_message(
            self.sample_episode_info, self.sample_articles
        )
...
class TestLineBroadcaster(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_articles = [{"title": "日経平均株価が大幅上昇", "sentiment_label": "Positive"}]
    ...
    def test_broadcast_with_flex_fallback(self, mock_post):
        ...
        result = self.broadcaster.broadcast_podcast_notification(
            self.sample_episode_info, self.sample_articles, use_flex=True
        )
...
class TestImageAssetManager(unittest.TestCase):
    ...
    def test_cache_key_generation(self):
        """キャッシュキー生成テスト"""
        cache_key = self.asset_manager._generate_cache_key(self.sample_episode_info, "thumbnail")
    ...
    def test_cache_validation(self):
        """キャッシュ有効性確認テスト"""
        # 有効なキャッシュ
        valid_cache = {"expires_at": datetime.now().timestamp() + 3600}  # 1時間後
        self.assertTrue(self.asset_manager._is_cache_valid(valid_cache))

        # 期限切れキャッシュ
        expired_cache = {"expires_at": datetime.now().timestamp() - 3600}  # 1時間前
...
class TestNotificationScheduler(unittest.TestCase):
    ...
    def test_schedule_stats(self):
        ...
        self.scheduler.schedule_podcast_notification(
            self.sample_episode_info, self.sample_articles, priority=NotificationPriority.HIGH
        )
