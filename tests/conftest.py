...
def test_config(temp_dir):
    ...
    return AppConfig(
        ...
        ai=AIConfig(gemini_api_key="test_api_key"),
        database=DatabaseConfig(url="sqlite:///:memory:", echo=False),
        logging=LoggingConfig(level="DEBUG", file_enabled=True, file_path=str(log_path)),
    )
...
def create_test_article(
    title: str = "テスト記事", url: str = "https://test.com/article", source: str = "Test", **kwargs
):
    """テスト用記事データ作成ヘルパー"""
    article_data = {
        "title": title,
...
def create_test_ai_analysis(
    sentiment_label: str = "Neutral", sentiment_score: float = 0.5, **kwargs
):
    """テスト用AI分析結果作成ヘルパー"""
    analysis_data = {
